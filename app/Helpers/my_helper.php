...
function getDesa($code, $type = 'kelurahan')
{
    $apiModel = new ApiModel();
    if ($type == 'kelurahan') {
        $codes = substr($code, 0, 6);
    } else {
        $codes = substr($code, 0, 4);
    }

    $api = $apiModel->getApi("https://ibnux.github.io/data-indonesia/$type/", "$codes.json");

    if ($type == 'kelurahan') {
        $key = array_search($code, array_column($api, 'id'));
...
function getMonthKuota()
{
    $db = db_connect();
    $months = $db->table('kuota')->select('created_at, approved_at')->groupBy('created_at')->get()->getResult();
    foreach ($months as $row) {
        $bln = date('m', strtotime($row->created_at));
        $bulan[$bln] = getBulan($bln) . ' ' . ($row->approved_at != null ? "✔" : '');
    }

    return $bulan ?? ['' => 'Tidak ada data'];
}

function getYearKuota()
{
    $db = db_connect();
    $years = $db->table('kuota')->select('created_at')->groupBy('created_at')->get()->getResult();
    foreach ($years as $row) {
        $tahun = date('Y', strtotime($row->created_at));
        $year[$tahun] = $tahun;
    }

    return $year ?? ['' => 'Tidak ada data'];
}

function getYearHargaBahan()
{
    $db = db_connect();
    $years = $db->table('harga_bahan')->select('tanggal')->groupBy('tanggal')->get()->getResult();
    foreach ($years as $row) {
...
function getMonthHargaBahan()
{
    $db = db_connect();
    $months = $db->table('harga_bahan')->select('tanggal')->groupBy('tanggal')->get()->getResult();
    foreach ($months as $row) {
        $bln = date('m', strtotime($row->tanggal));
        $bulan[$bln] = getBulan($bln);
    }

    return $bulan ?? ['' => 'Tidak ada data'];
}

function getKodeKec($filter = 0)
{
    $apiModel = new ApiModel();
    $api = $apiModel->getApi('https://ibnux.github.io/data-indonesia/kecamatan/', '7108.json');
    $data = [];
    switch ($filter) {
        case 1:
            foreach ($api as $row) {
...
function getWeek($date, $getweek = false)
{
    $tgl = (int)date('d', strtotime($date));
    $month = date('t', strtotime($date));
    $year = date('Y', strtotime($date));
    $last = (int)date('t', mktime(0, 0, 0, $month, 1, $year));
    $weeks = 0;
    for ($i = 1; $i <= $last; $i++) {
        $hari = substr($date, 0, 7) . '-' . sprintf('%02d', $i);
    ...
    if ($getweek === true) {
        foreach ($weekends as $key => $weekend) {
            $key++;
            $minggu = $tgl > $weekend ? $key + 1 : $key;
            if ($tgl <= $weekend) break;
        }
        return $minggu;
    }
    return $lastday < $last ? $weeks + 1 : $weeks;
}
