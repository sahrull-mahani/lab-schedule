...
class VisitorTracker
{
    ...
    function track_visitor()
    {
        ...
        if (isset($track_visitor) && $track_visitor === TRUE) {
            ...
            $date = date('Y-m-d');
            $unique = $this->db->table('visitor_log')->where(['ip_address'=>$ip, 'user_agent'=>$user_agent, 'access_date'=>$date])->get()->getResult();
            if (count($unique) == 0) {
                if ($ip != '0.0.0.0') {
                    $data = [
                        'ip_address' => $ip,
                    ...
                    $this->db->table('visitor_log')->insert($data);
                }
            }
        }
    }
